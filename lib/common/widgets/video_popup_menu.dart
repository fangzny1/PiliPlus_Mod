...
import 'package:PiliPlus/common/widgets/keyword_splitter_dialog.dart';
      ...
      itemBuilder: (context) =>
          [
                ...
                if (videoItem is! SpaceArchiveItem) ...[
                  ...
                  _VideoCustomAction(
                    '此类内容过多',
                    const Icon(Icons.do_disturb_alt_outlined, size: 16),
                    () async {
                      try {
                        SmartDialog.showLoading(msg: '处理中');
                        final blockRes = await VideoHttp.relationMod(
                          mid: videoItem.owner.mid!,
                          act: 5,
                          reSrc: 11,
                        );
                        if (blockRes.isSuccess) {
                          SmartDialog.showToast(
                            '已拉黑:${videoItem.owner.name}',
                          );
                        }
                        if (videoItem case final RcmdVideoItemAppModel appItem) {
                          final tp = appItem.threePoint;
                          if (tp?.feedbacks != null &&
                              tp!.feedbacks!.isNotEmpty) {
                            await VideoHttp.feedDislike(
                              feedbackId: tp.feedbacks!.first.id,
                              id: appItem.param!,
                              goto: appItem.goto!,
                            );
                          }
                        } else if (videoItem.bvid?.isNotEmpty == true) {
                        ...
                        SmartDialog.dismiss();
                        final currentKeywords = GStorage.setting.get(
                          SettingBoxKey.banWordForRecommend,
                          defaultValue: '',
                        ) as String;
                        if (safeCtx.mounted) {
                          final result = await showKeywordSplitterDialog(
                            context: safeCtx,
                          ...
                          if (result != null) {
                            ...
                            RecommendFilter.enableFilter = result.isNotEmpty;
                            SmartDialog.showToast('关键词已保存');
                          }
                        }
                        onRemove?.call();
                      } catch (e) {
